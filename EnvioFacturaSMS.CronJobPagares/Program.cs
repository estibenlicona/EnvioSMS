using EnvioFacturaSMS.Applications.CQRS.Pagares;
...
namespace EnvioFacturaSMS.CronJobPagares
{
    public class Program
    {
        public static async Task Main()
        {
            ...
            using (var scope = Container.Services.CreateScope())
            {
                ...
                try
                {
                    EnviarPagareQuery SmsPagare = new EnviarPagareQuery();

                    var _config = scope.ServiceProvider.GetRequiredService<IConfiguration>();
                    SmsPagare.TimesTamp = (Environment.GetEnvironmentVariable("FECHA_ENVIO") != null) ? DateTime.Parse(Environment.GetEnvironmentVariable("FECHA_ENVIO")) : _config.GetValue<DateTime>("FechaEnvio");

                    var _mediator = scope.ServiceProvider.GetRequiredService<IMediator>();
                    await _mediator.Send(SmsPagare);
        ...
        public static IHostBuilder CreateHostBuilder() =>
            Host.CreateDefaultBuilder()
                .ConfigureServices((hostContext, services) =>
                {
                    ...
                    services.AddTransient<IPagareService, PagareService>();
